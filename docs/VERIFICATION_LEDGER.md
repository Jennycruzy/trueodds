...
### Ledger status as of 2026-07-08 (Phase 6 + pre-listing hardening + scanner complete)
All GATE 0–8-required facts are verified with real evidence above, including a real, correctly-verified X Layer mainnet anchor transaction (§16.1) after catching and fixing a genuine false-positive in the anchor verification logic itself. Other open items remain explicitly flagged and **not assumed**: the Payment SDK settlement token (§7), the Kalshi fee-schedule PDF direct fetch blocked by HTTP 429 from this workspace, future sports draw/injury/projection enhancements, funded execution/risk controls, and the hosted public calibration page.
